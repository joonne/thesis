...
Suosittelujärjestelmä generoi ennustettuja arvioita $\hat{r}_{ui}$ testisetille $\tau$ käyttäjä-tuote pareja $(u,i)$ joille todelliset arviot $r$ tunnetaan.
ennustettujen ja todellisten arvioiden välinen RMSE on annettu:

\begin{equation}
RMSE=\sqrt{\frac{1}{|\tau|} \sum_{(u,i)\in\tau}(\hat{r}_{ui}-r_{ui})^2}
\end{equation}

esimerkiksi toetutuksessa käytetty...
