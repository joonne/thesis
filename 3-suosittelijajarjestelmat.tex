...
Alla oleva ohjelma esittelee tuotepohjaisen yhteisösuodatuksen idean jokaiselle uudelle käyttäjälle.

\begin{lstlisting}[caption=Tuotepohjaisen yhteisösuodatuksen algoritmi 1/2 \cite{gorakala15}]
...
val similarItems: Seq[Similarity] = items.map { item1 =>
	items.map { item2 =>
		Similarity (item1, item2, calculateSimilarity(item1, item2))
	}
}
...
\begin{lstlisting}[caption=Tuotepohjaisen yhteisösuodatuksen algoritmi 2/2 \cite{gorakala15}]
...
val similars = users.map { user =>
	user.purchases.map { purchase =>
		findSimilarItem(purchase, similarities)
	}
}
...
\begin{lstlisting}[caption=Käyttäjäpohjainen yhteisösuodatus algoritmi osa \cite{gorakala15}]
