...
Ensimmäinen suosittelijajärjestelmä yritti matkia tätä käytöstä etsimällä verkkoyhteisöstä suosituksia aktiiviselle käyttäjälle.
...
Seuraavassa tuotetta kuvataan symbolilla $i$ (item) ja käyttäjää kuvataan symbolilla $u$ (user).
...
Yhteisösuodatus analysoi käyttäjien välisiä suhteita ja tuotteiden välisiä riippuvuuksia tunnistaakseen uusia käyttäjä-tuote -assosiaatioita \cite{korenBellVolinsky09}.
Päätelmä siitä, että käyttäjät voisivat pitää samasta laulusta, koska molemmat kuuntelevat muita samankaltaisia lauluja on esimerkki yhteisösuodatuksesta \cite{ryza15}.

Koska yhteisösuodatuksessa suosittelu perustuu pelkästään käyttäjän arvosteluihin tuotteesta, yhteisösuodatus kärsii ongelmista jotka tunnetaan nimillä \textit{uusi käyttäjäongelma} ja \textit{uusi tuoteongelma} \cite{gorakala15}.
Ellei käyttäjä ole arvostellut yhtään tuotetta, algoritmi ei kykene tuottamaan myöskään yhtään suositusta.
Muita yhteisösuodatuksen haasteita ovat \textit{kylmä aloitus} sekä \textit{niukkuus} (sparsity).
...
\subsection{Muistiperustainen yhteisösuodatus}
...
Ohjelmat \ref{ibcf_alg}, \ref{ubcf_alg1} ja \ref{ubcf_alg2} on luonnosteltu tekstipohjaisten selitysten perusteella käyttäen Scala-syntaksia.
...
Alla oleva ohjelma \ref{ibcf_alg} esittelee tuotepohjaisen yhteisösuodatuksen idean jokaiselle uudelle käyttäjälle.

\begin{lstlisting}[caption=Tuotepohjaisen yhteisösuodatuksen algoritmi \cite{gorakala15}, label={ibcf_alg}]
		// Jokaiselle kahdelle tuotteelle, mittaa kuinka samankaltaisia ne ovat.
		...
		val similarItems: Seq[Similarity] = items.map { i1 =>
			items.map { i2 => Similarity (i1, i2, getSimilarity(i1, i2)) }
		}
...
Askeleet jokaiselle uudelle käyttäjälle käyttäjäpohjaisessa yhteisösuodatuksessa on esitetty ohjelmissa \ref{ubcf_alg1} ja \ref{ubcf_alg2}.

\begin{lstlisting}[caption=Käyttäjäpohjaisen yhteisösuodatuksen algoritmi osa 1/2 \cite{gorakala15}, label={ubcf_alg1}]
		// Mittaa jokaisen käyttäjän samankaltaisuus uuteen käyttäjään.

		case class Similarity(userId1: Int, userId2: Int, score: Double)
...
\begin{lstlisting}[caption=Käyttäjäpohjaisen yhteisösuodatuksen algoritmi osa 2/2 \cite{gorakala15}, label={ubcf_alg2}]
