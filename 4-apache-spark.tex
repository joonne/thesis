...
Kuvassa \ref{dag} on havainnollistettu DAG:in asyklistä datavirtaa.
...
Yleisin tapa on käyttää $SparkSession$:in tarjoamaa $read$-funktiota ja osoittaa Spark joihinkin tiedostoihin tiedostojärjestelmässä, kuten ohjelmassa \ref{json_example} esitettyyn $json$-tiedostoon:
...
Ohjelmassa \ref{spark_read_example} on esitetty Datasetin luominen käyttäen $spark.read$ -funktiota.

\begin{lstlisting}[caption={Uuden Dataset-olion luominen käyttäen read-funktiota}, label={spark_read_example}]
...
Seuraavassa ohjelmassa \ref{dataset_example} luodaan uusi Dataset lukemalla $json$-tiedosto tiedostojärjestelmästä.
Seuraavaksi luodaan uusi Dataset muunnoksen kautta.
Olion kloonaamiseksi käytetään case class :n $copy$-metodia, koska case class -instanssit ovat muuttumattomia ja näin ollen copy-metodi on ainoa tapa kloonata eli luoda uusi muuttumaton (immutable) instanssi siitä.
...
Alla oleva kuva \ref{dataframe} esittää hahmotelman DataFrame:sta.
...
Kooditasolla Scala-toteutuksessa DataFrame on riveistä rakentuva Dataset. \cite{spark-programming-guide} \cite{spark_sql_dataframes}
...
\begin{lstlisting}[caption={DataFrame:n luominen käyttäen read-funktiota}, label={dataframe_example}]
...
Yllä olevassa ohjelmassa \ref{dataframe_dsl_example} valitaan luodusta DataFrame:sta vain $name$ -sarake ja tulostetaan se konsoliin. \cite{spark-sql-programming-guide}
...
Tässä työssä tutustutaan MLlib-kirjaston tarjoamaan ALS-kirjastoon, jolla suositukset tarjoava koneoppimismalli opetetaan. \cite{mllib}
