...
Klusterin konfigurointi piti tehd‰ k‰yt‰nnˆss‰ kolmannen osapuolen \cite{levelup} videon perusteella, sill‰ AWS:n omat ohjeistukset eiv‰t olleet riitt‰v‰t, tai ainakin ne koettiin vaikeaselkoisiksi.
...
Yll‰ olevassa kuvassa \ref{emr_configuration} on esitetty klusterin konfiguroinnin www-k‰yttˆliittym‰, johon p‰‰st‰‰n k‰siksi AWS:n konsolin kautta.
...
Yll‰ olevassa ohjelmassa \ref{create_cluster_example} pystytet‰‰n EMR klusteri AWS:n komentorivityˆkalun avulla.
...
T‰ss‰ tyˆss‰ k‰ytett‰v‰ MovieLens 20M -aineisto sis‰lt‰‰ 20 000 000 (kaksikymment‰ miljoonaa) arviota, jotka ovat antaneet 138 000 k‰ytt‰j‰‰ 27 000 elokuvalle.
...
\begin{lstlisting}[caption=Sovelluksen k‰yttˆˆnotto klusterissa,language=sh, label={spark_submit_example}]
$ spark-submit movielens-recommendations_2.11-1.0.jar
\end{lstlisting}

Yll‰ olevassa ohjelmassa \ref{spark_submit_example} on esimerkki ohjelman k‰yttˆˆnotosta Spark-klusterissa.
...
Omassa opetuksessa eroavaisuuksina olivat t‰m‰n hetken l‰hin vastaava datasetti, joka ei ollut aivan niin suuri kuin tutkimuksessa k‰ytetty, myˆs opetusaineistojen suhde oli hieman eri, sill‰ oman toteutuksen RMSE-validointi tarvitsi myˆs oman osansa datasta.
...
Ohjelmassa \ref{prediction} haetaan henkilˆkohtaiset suositukset k‰ytt‰m‰ll‰ mallin $predict$-metodia, joka ottaa parametrinaan mahdollisten elokuvien joukon.
