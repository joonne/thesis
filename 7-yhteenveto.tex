...
Jatkuvasta maksujen syntymisestä ei varsinaisesti kommunikoitu missään vaiheessa, vaan asiaan havahtui vasta laskun saavuttua.
...
Varsinaisen koodin testailu ja integrointi EMR:n kanssa oli kuitenkin inkrementaalista, joten sulkeutunut liukuhihna olisi ollut melkoinen hidaste.
...
Oikeampi tai ainakin edullisempi keino työn toteuttamiseen olisi ollut pystyttää liukuhihna, ajaa tarvittavat eräajot ja tuloksien kirjaamisen jälkeen pysäyttää liukuhihna, jolloin varatut resurssit olisi vapautettu.
...
\begin{itemize}
	\item EMR-liukuhihna voisi mahdollisesti mennnä ``nukkumaan'', kuten jotkut muut pilvipalvelut toimivat. Tähän ominaisuuteen tarvittaisiin jonkinlaista logiikkaa tunnistamaan, ettei koneella ole ajossa mitään tähdellistä.
...
Oletusarvoisesti EMR haluaa kirjoittaa lokit S3-buckettiin ja tämä saattaa helposti johtaa Free Tier -tilauksen pyyntöjen rajan ylittämiseen.
...
Tämä etsintä voitaisiin toteuttaa esimerkiksi ympäristömuuttujien avulla, joten aivan kaukaahaettu teoria se ei olisi.
...
Sulkeuma on funktionaalisissa ohjelmointikielissä käytetty ominaisuus, joka tarkoittaa funktion kykyä viitata leksikaalisen näkyvyysalueensa parametreihin.
...
Kuten aiemmin todettiin, on mahdollista ladata ennakkoon koulutettu malli Spark-sovellukseen.
...
Olisi mielenkiintoista tutkia vielä, että millä omien suositusten määrällä suositukset alkaisivat osua enemmän kohdalleen.
Lähestymistapa tässä voisi olla esimerkiksi sellainen, että jätettäisiin joitain suosikkielokuvia tarkoituksella arvostelematta ja tarkkailtaisiin, että missä kohdassa järjestelmä rupeaisi ehdottamaan juuri näitä elokuvia.
