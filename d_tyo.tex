...
Recommendation denotes a task in which a tool is measuring similarity of items. It refers to an algorithm that calculates the similarity of the items to be compared. This can performed by comparing users to other users or comparing items into other items. Different similarity functions are described in Section 2.2.

A recommendation engine is a system that includes all the necessary steps to retrieve the actual recommendation result. This includes acquiring, cleaning and preprocessing of the data, recommendation algorithm and finally the interpretation of obtained results. Most of these steps are covered in Section 2.1.

A recommendation system can be used in tasks such as increasing the number of items sold, selling more diverse items, increasing user satisfaction, increasing user loyalty or understanding better what the user wants \cite{ricci11}. In addition, a number of different functions exist that the user might expect a recommendation system to offer. Some of these are similar to the ones mentioned above, thus considered as core functions. Others include tasks such as recommending a sequence of items. \cite{ricci11}

The perfect recommendation system must balance between the needs of service provider and the customer. \cite{ricci11}

GroupLens was a pioneer in recommendation systems along with BookLens and MovieLens. In addition to this they also released data sets which, aside of recommendation, was also pioneering in the field since data sets were not that common for benchmarking or just trying out new technologies. \cite{aggarwal16}
...
%\section{Building own recommendation engine}
%
%Simplest form of recommendation might be just issuing SQL queries to suitable tables. ElasticSearch could be the next step since it needs some additional steps before using it. Either pointing it to the existing database or populating the ElasticSearch NoSQL database with the data and then start to issue queries against it. Writing own recommendation engine from the scratch requires considerably more development, many options available such as Spark and Hadoop.
%
%\section{3rd party alternatives}
%
%When recommendation is not the core business for a company, it might be better to get the recommendation as a service from service providers such as AWS, Azure ML and IBM Watson. As a side note, even using these ready technologies needs considerably deep domain knowledge, knowledge about problem to be solved and everything related to the task. Another option is to purchase this recommendation service from a software company.
%
...
% Recommendation system must have some sort of understanding about the items to be able to recommend something. Several methods have been proposed and studied to address this problem \cite{gorakala15} \cite{ricci11} \cite{burke02}. Majority of these techniques are discussed in the following sections.
