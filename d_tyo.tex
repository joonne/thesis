...
\begin{lstlisting}[caption=User-Based Collaborative Filtering algorithm \cite{gorakala15}]
  ...
  2. Identify the most similar users. The options are: Take account of the top k users (k-nearest_neighbors) Take account of the users whose similarity is above a defined threshold
	...
	val mostSimilarUsers = similarities.filter { user => user.score > 0.8
	}
  ...
  mostSimilarUsers.map { user =>
    user.purchases.map { purchase =>
      val purchases = mostSimilarUsers.map { usr =>
        usr.purchases.filter(p => p.id === purchase.id)
      }
...
- MovieLens ml-latest-small dataset. Describe the dataset.

- What is MovieLens?

- We used RDD based API since dataset API is not yet available on MLlib.

- Loading data can be done with dataset API but the recommendation with ALS needs to be done with RDDs still.
