...
Essentially it is an immutable, partitioned collection of elements in Spark that can be distributed across multiple machines in a cluster. \cite{spark-rdd}
...
The most common way is to make use of SparkSession's read function and point Spark to some files on storage systems:
...
If we would omit the casting with the $as$ then we would end up creating a DataFrame and the schema of that would be guessed by Spark.
