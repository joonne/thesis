...
Tässä tekstissä keskitytään vain yhteen suosittelutekniikoiden luokkkaan: \textit{yhteisösuodatukseen}.
Tätä menetelmää käytetään \textit{Apache Spark} -sovelluskehyksessä, joka on yksi tekstin tärkeimmistä osa-alueista.
...
Tekstiin liittyvän työn päämääränä on tutustua Apache Spark-sovelluskehykseen sekä \textit{Scala}-ohjelmointikieleen ja toteuttaa näiden teknologioiden avulla suosittelujärjestelmä siten, että toteutettava järjestelmä toimisi jossain pilvipalvelussa.
Pilvipalvelun ajateltiin tarjoavan laskentaresursseja käyttöön helposti ja edullisemmin, kuten tapahtuikin.
Alustaksi valikoitui Amazonin tarjoama \textit{Amazon Web Services (AWS)}, sillä siitä oli hieman aiempaa kokemusta ja sen osaamisesta on varmasti hyötyä tulevaisuudessakin.

% aws

AWS  on Amazonin tarjoama kokoelma pilvilaskentaan (cloud computing) tarkoitettuja tai sitä avustavia palveluita.
Työn kriteeriksi valikoitui Spark-sovelluksen ajaminen pilvipalvelussa ja Amazonin palveluista tässä auttoivat \textit{EMR (Elastic Map Reduce)} sekä \textit{S3 (Simple Storage Service)}.
